...
> この記事は、2026年8月11日に Qiita で公開した記事の転載です。原文は以下。
> note 版は、エンジニア以外の方にも読んでいただけるよう、少し加筆しています。
...
対象は前編と変わりません。**AI駆動開発の話についていけていない**と感じているすべての人——具体的には、次の2種類の方に向けて書いています。

- **エンジニア**：プログラミングは普通にできる。でもAIエージェントを使った開発の経験はほぼゼロ
- **PdM・ディレクター・マネージャー**：自分でコードは書かないけれど、チームが「Claude Code が」「MCP が」と話しているのを、わかったふりで聞いている

**Skills？ オーケストレーター？ ハーネス？ ——という状態からの脱出**がゴールです。後編も、設定ファイルの書き方は一行も出てきません。

ただ、正直に先に言っておきます。**後編は前編より、概念そのものが難しくなります。** ハーネス、検証ループ、オーケストレーション——どれも目に見える機能というより、「AIをどう働かせるか」という設計の話だからです。

とはいえ、コードを書かない立場の方ほど、ここが効いてきます。「この作業、AIに丸ごと任せられますよね？」が正しいのか無茶なのかは、**AIをどこまで自走させられるのか**——まさに後編で扱う話——を知らないと判断できないからです。

なので後編も、「これは何のためのものか」が頭の中に絵として浮かぶ状態を最優先に、前編と同じくイラストとたとえ話を多めにして書いています。
